...
kubectl delete statefulset mysql-secondary mysql-primary -n "$NAMESPACE" --ignore-not-found

echo "[2/5] Deleting Services..."
kubectl delete -f service.yaml -n "$NAMESPACE" --ignore-not-found

echo "[3/5] Deleting ConfigMaps..."
kubectl delete configmap mysql-primary-config mysql-secondary-config mysql-replication-init -n "$NAMESPACE" --ignore-not-found
